...
template<typename T>
QSMatrix<T>::QSMatrix(unsigned _rows, unsigned _cols, const T* _arr) {
	mat.resize(_rows);

	for (unsigned i=0; i<mat.size(); i++) {
		mat[i].resize(_cols, 0.0);
		mat[i].assign(_arr+_cols*i, _arr+_cols*i + _cols);
	}
...
template<typename T>
QSMatrix<T> QSMatrix<T>::operator*(const QSMatrix<T>& rhs) {
	unsigned r_rows = rhs.get_rows();
	unsigned r_cols = rhs.get_cols();
	QSMatrix result(rows, r_cols, 0.0);
